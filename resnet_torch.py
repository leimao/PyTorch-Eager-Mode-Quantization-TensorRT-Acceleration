...
def create_model(num_classes=10, resnet=resnet18):
...
def main():
    ...
    onnx_model_filename = "resnet_cifar10.onnx"
    quantized_model_filename = "resnet_quantized_cifar10.pt"
    quantized_onnx_model_filename = "resnet_quantized_cifar10.onnx"
    model_filepath = os.path.join(model_dir, model_filename)
    ...
    model = create_model(num_classes=num_classes, resnet=resnet)
    ...
    torch.onnx.export(model,
                      torch.rand(size=(1, 3, 32, 32)).to(cuda_device),
                      onnx_model_filepath,
    ...
    for module_name, module in fused_model.named_children():
        if "layer" in module_name:
            for basic_block_name, basic_block in module.named_children():
                fusion = []
                if resnet == resnet18:
                    fusion = [["conv1", "bn1", "relu1"], ["conv2", "bn2"]]
                elif resnet == resnet50:
                    fusion = [["conv1", "bn1", "relu1"], ["conv2", "bn2"],
                              ["conv3", "bn3"]]
                else:
                    raise ValueError("Unsupported ResNet model!")
                torch.ao.quantization.fuse_modules(basic_block,
                                                   fusion,
                                                   inplace=True)
                for sub_block_name, sub_block in basic_block.named_children():
                    if sub_block_name == "downsample":
                        torch.ao.quantization.fuse_modules(sub_block,
                                                           [["0", "1"]],
                                                           inplace=True)
    ...
    # quantized_model = QuantizedResNet(model_fp32=model)
    ...
    # https://discuss.pytorch.org/t/quantization-using-one-at-a-time-analysis/154861/2
    # This is unfortunately difficult to do with the PyTorch module implemented in the third-party library.
    # The following code will not work.
    # quantized_model.model_fp32.fc.qconfig = None

    # Print quantization configurations
    print(quantized_model.qconfig)

    # https://pytorch.org/docs/master/torch.ao.quantization.html#torch.ao.quantization.prepare
    ...
    # The above steps, including torch.ao.quantization.prepare, calibrate_model, and torch.ao.quantization.convert, are also equivalent to
    # quantized_model = torch.ao.quantization.quantize(model=quantized_model, run_fn=calibrate_model, run_args=[train_loader], mapping=None, inplace=False)
    ...
    torch.onnx.export(quantized_model,
                      torch.rand(size=(1, 3, 32, 32)).to(cpu_device),
